...
public class LoginControllerTests {
    ...
    private User mockUser(String username, String hashedPassword) {
        User user = mock(User.class);
        when(user.hashedPassword).thenReturn(hashedPassword);
        when(user.token(secret)).thenReturn("mockedToken");
        return user;
    }
    ...
    public void login_ValidCredentials_ShouldReturnToken() {
        ...
        String hashedPassword = "hashedValidPassword";

        LoginRequest request = createLoginRequest(username, password);
        User user = mockUser(username, hashedPassword);
        ...
        assertEquals("Token should match", "mockedToken", response.token);
    }
    ...
    public void login_InvalidPassword_ShouldThrowUnauthorized() {
        ...
        when(User.fetch(username)).thenReturn(user);
        when(Postgres.md5(password)).thenReturn("hashedInvalidPassword");
