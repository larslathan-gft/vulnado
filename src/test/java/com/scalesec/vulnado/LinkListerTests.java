...
public class LinkListerTests {
    ...
    @Test
    public void getLinksV2_ShouldReturnListOfLinks_WhenUrlIsValid() throws BadRequest, IOException {
        // Arrange
        List<String> expectedLinks = List.of("http://example.com", "http://test.com");
        Document mockDocument = mockJsoupDocumentWithLinks(expectedLinks);
        Mockito.mockStatic(Jsoup.class);
    ...
    public void getLinksV2_ShouldThrowBadRequest_WhenPrivateIPIsUsed() throws BadRequest {
        // Act
        LinkLister.getLinksV2("http://192.168.1.1");
    }

    @Test(expected = BadRequest.class)
    public void getLinksV2_ShouldThrowBadRequest_WhenMalformedUrlIsProvided() throws BadRequest {
        // Act
        LinkLister.getLinksV2("malformed-url");
    }

    @Test(expected = BadRequest.class)
    public void getLinksV2_ShouldThrowBadRequest_WhenJsoupFails() throws BadRequest, IOException {
        // Arrange
        Mockito.mockStatic(Jsoup.class);
        when(Jsoup.connect("http://mockurl.com").get()).thenThrow(new IOException("Connection failed"));

        // Act
        LinkLister.getLinksV2("http://mockurl.com");
    ...
    @Test
    public void getLinksV2_ShouldHandleNonPrivateIPCorrectly() throws BadRequest, IOException {
        // Arrange
        List<String> expectedLinks = List.of("http://example.com", "http://test.com");
        Document mockDocument = mockJsoupDocumentWithLinks(expectedLinks);
        Mockito.mockStatic(Jsoup.class);
        when(Jsoup.connect("http://8.8.8.8").get()).thenReturn(mockDocument);

        // Act
        List<String> actualLinks = LinkLister.getLinksV2("http://8.8.8.8");
