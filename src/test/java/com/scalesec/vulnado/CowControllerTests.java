...
public class CowControllerTests {
    ...
    public void cowsay_NullInput_ShouldReturnDefaultMessage() {
        try (MockedStatic<Cowsay> mockedCowsay = mockStatic(Cowsay.class)) {
            // Mocking the Cowsay.run method
            mockedCowsay.when(() -> Cowsay.run("I love Linux!")).thenReturn("Mocked Cowsay Output for Null Input");

            String result = cowController.cowsay(null);

            assertEquals("Expected mocked output for null input", "Mocked Cowsay Output for Null Input", result);
        }
    }
}
