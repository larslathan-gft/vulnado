...
public class UserTests {
    ...
    public void fetch_ShouldReturnUserWhenUserExists() throws Exception {
        ...
        ResultSet mockResultSet = createMockResultSet("1", username, "hashedpassword");

        when(mockConnection.createStatement()).thenReturn(mockStatement);
        when(mockStatement.executeQuery(anyString())).thenReturn(mockResultSet);
        Mockito.mockStatic(Postgres.class).when(Postgres::connection).thenReturn(mockConnection);
