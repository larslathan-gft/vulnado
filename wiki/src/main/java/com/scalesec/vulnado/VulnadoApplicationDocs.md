...
```mermaid
flowchart TD
    A("VulnadoApplication") --> B["Postgres.setup()"]
    A --> C["SpringApplication.run(VulnadoApplication, args)"]
```

## Insights
- The class is annotated with `@SpringBootApplication` which indicates that it is a Spring Boot application.
